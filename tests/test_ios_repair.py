...
from reproof.ios_build import PRODUCT_FILE
...
class IosRepairTests(unittest.TestCase):
    def setUp(self):
        ...
        with (self.app/'Info.plist').open('wb') as f:plistlib.dump({'CFBundleIdentifier':'io.reproof.sample.ios','ReproBuildID':'original-build'},f)
    ...
    def build(self,source,output,simulator_id,**kwargs):
        ...
        with (app/'Info.plist').open('wb') as f:plistlib.dump({'CFBundleIdentifier':'io.reproof.sample.ios','ReproBuildID':'patched-build'},f)
    ...
    def test_patch_uses_original_runner_and_keeps_root_source(self):
        ...
        with patch('reproof.ios_repair.build_ios',self.build),patch('reproof.ios_repair.run_logic_test',return_value={'passedTests':1}):
