#ifndef REPRO_ANDROID_OWNERSHIP_H
#define REPRO_ANDROID_OWNERSHIP_H
...
static int producer_directory_fd, device_directory_fd;
...
static int owner_record(int descriptor, const char *state) {
    ...
    if (length <= 0 || (size_t)length >= sizeof(buffer) || !original_directory() ||
        !owned_named(descriptor, name, 1) || !original_locks() || ftruncate(descriptor, 0)) return 0;
    size_t offset = 0;
...
static int start_owner(CFDictionaryRef config, const int descriptors[9]) {
    ...
    if (owner.work_path[0] != '/' || !realpath(owner.work_path, canonical) || strcmp(canonical, owner.work_path) ||
        !original_directory() || !original_locks() || !owned_named(owner.start_fd, "start.json", 1) ||
...
static void finish_owner(int exit_code) {
    ...
    printf("{\"schemaVersion\":1,\"contextDigest\":\"%s\",\"status\":\"%s\",\"hostClientStopped\":true,\"deviceCleanupConfirmed\":false}\n",
        owner.context_digest, exit_code == 0 ? "succeeded" : "failed");
